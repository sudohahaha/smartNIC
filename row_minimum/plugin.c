...
#define STATE_TABLE_SIZE 0xC34F /* 16777200 state table entries available */
...
volatile __emem __export uint32_t global_semaphores[STATE_TABLE_SIZE + 1] = {VAL_32768X,VAL_16384X, VAL_512X, VAL_256X, VAL_64X, VAL_16X};
...

{
    ...
    if (i == BUCKET_SIZE){
        mem_read32(heap_arr_r, state_hashtable[update_hash_value].row, sizeof(heap_arr_r));
...
//        // One by one extract an element from heap
        ...
        mem_write32(&tmp_b_info, &state_hashtable[update_hash_value].row[minimum_i], sizeof(tmp_b_info));
        mem_write32(&key_val_rw0,&state_hashtable[update_hash_value].entry[minimum_i].key0, sizeof(key_val_rw0));
        mem_write32(&key_val_rw1,&state_hashtable[update_hash_value].entry[minimum_i].key1, sizeof(key_val_rw1));
        mem_write32(&key_val_rw2,&state_hashtable[update_hash_value].entry[minimum_i].key2, sizeof(key_val_rw2));
        mem_write32(&key_val_rw3,&state_hashtable[update_hash_value].entry[minimum_i].key3, sizeof(key_val_rw3));
        mem_incr32(&ejection_count);
        
        //export the largest -> potential hh -> hh buffer -> minimize the read
